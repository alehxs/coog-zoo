...
function ManageEmployees({ employeeId, setEmployeeId, showSidebar }) {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const data = await fetchWithRetry('https://coog-zoo.onrender.com/employees');
        setEmployees(data);
      } catch (err) {
  ...
  useEffect(() => {
    const fetchExhibits = async () => {
      try {
        const data = await fetchWithRetry('https://coog-zoo.onrender.com/exhibits');
        setExhibits(data);
      } catch (err) {
        setError(err.message);
      }
  ...
  const updateEmployee = async () => {
    ...
    try {
      const response = await fetchWithRetry(
        `https://coog-zoo.onrender.com/update-employee?id=${updateData.id}`,
        {
          method: 'PUT',
  ...
  const handleDeleteEmployee = async () => {
    try {
      const response = await fetchWithRetry(
        `https://coog-zoo.onrender.com/remove-employee?id=${employeeId}`,
        { method: 'DELETE' }
      );
  ...
  const validateAndAddEmployee = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedResponse = await fetch('https://coog-zoo.onrender.com/employees');
