...
const Membership = () => {
  ...
  useEffect(() => {
    if (isCustomer && userEmail) {
      const fetchMembershipDetails = async () => {
        try {
          const customerResponse = await fetch(`https://coog-zoo.onrender.com/profile?email=${userEmail}&type=customer`);
          if (!customerResponse.ok) throw new Error('Failed to fetch user profile');
          const customerData = await customerResponse.json();
          const userId = customerData.profile.ID;

          const response = await fetch(`https://coog-zoo.onrender.com/membership-details?userId=${userId}`);
  ...
  const handleUpgrade = async (tier) => {
    ...
    try {
      ...
      const response = await fetch('https://coog-zoo.onrender.com/upgrade-membership', {
        method: 'POST',
        headers: {
      ...
      const updatedResponse = await fetch(`https://coog-zoo.onrender.com/membership-details?userId=${userId}`);
